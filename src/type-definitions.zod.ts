...
export const falsePositiveCount = z.number().refine(
  (val) => val >= 0,
  (val) => ({
    message: `False Positive ${val} must be greater than or equal to 0`,
  })
);
export const falseNegativeCount = z.number().refine(
  (val) => val >= 0,
  (val) => ({
    message: `False Negative ${val} must be greater than or equal to 0`,
  })
);
...
// Loading
const percentage = z.number().refine((val) => val >= 0 && val <= 100);
